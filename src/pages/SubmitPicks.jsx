...
const SubmitPicks = () => {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const pickPromises = Object.entries(picks).map(
        ([tierNumber, driverId]) => {
          const tier = tiersData.find(
            (t) => t.tier_number === parseInt(tierNumber),
          )
          return submitPicks({
            pick: {
  ...
  const allTiersPicked =
    tiersData?.length > 0 && Object.keys(picks).length === tiersData.length
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    {tier.drivers?.map((driver) => (
                      <DriverCard
                        key={driver.id}
                        driver={driver}
                        selected={picks[tier.tier_number] === driver.id}
                        onClick={() =>
                          setPicks((prev) => ({
                            ...prev,
                            [tier.tier_number]: driver.id,
                          }))
                        }
                        showPosition={false}
                      />
